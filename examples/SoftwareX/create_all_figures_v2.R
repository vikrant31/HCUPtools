...
# This script creates all 5 figures needed for the manuscript
...
cat("Creating all figures for Healthcare Analytics manuscript...\n\n")

# ============================================================================
# Figure 1: Package Architecture
# ============================================================================

cat("Creating Figure 1: Package Architecture...\n")
...
p1 <- ggplot() +
  ...
  annotate("rect", xmin = 0.15, xmax = 0.45, ymin = 0.4, ymax = 0.6, 
           fill = "#90CAF9", color = "black", linewidth = 1) +
  ...
  annotate("rect", xmin = 0.1, xmax = 0.9, ymin = 0.2, ymax = 0.35, 
           fill = "#64B5F6", color = "black", linewidth = 1) +
  ...
  annotate("rect", xmin = 0.1, xmax = 0.9, ymin = 0.05, ymax = 0.15, 
           fill = "#42A5F5", color = "black", linewidth = 1) +
...
cat("Creating Figure 2: Workflow Comparison...\n")
...
p2 <- ggplot(data.frame(x = c(0, 1), y = c(0, 1)), aes(x = x, y = y)) +
  ...
  annotate("rect", xmin = 0.05, xmax = 0.48, ymin = 0.05, ymax = 0.85,
           fill = "#FFEBEE", color = "#D32F2F", linewidth = 2) +
...
for (i in seq_len(7)) {
  p2 <- p2 +
    annotate("rect", xmin = 0.08, xmax = 0.45, ymin = y_positions[i] - 0.06,
             ymax = y_positions[i] + 0.02, fill = "white", color = "gray", linewidth = 0.5) +
  ...
  if (i < 7) {
    p2 <- p2 +
      annotate("segment", x = 0.265, xend = 0.265,
               ...
               arrow = arrow(length = unit(0.15, "cm")), color = "#D32F2F", linewidth = 1)
  }
}

p2 <- p2 +
  annotate("rect", xmin = 0.52, xmax = 0.95, ymin = 0.05, ymax = 0.85,
           fill = "#E8F5E9", color = "#388E3C", linewidth = 2) +
  annotate("text", x = 0.735, y = 0.82, label = "HCUPtools Workflow",
           size = 6, fontface = "bold", color = "#388E3C") +
  annotate("rect", xmin = 0.55, xmax = 0.92, ymin = 0.6, ymax = 0.75,
           fill = "white", color = "gray", linewidth = 0.5) +
  annotate("text", x = 0.735, y = 0.675, label = "1. download_ccsr()",
           size = 4.5, hjust = 0.5, fontface = "bold") +
  annotate("rect", xmin = 0.55, xmax = 0.92, ymin = 0.4, ymax = 0.55,
           fill = "white", color = "gray", linewidth = 0.5) +
  annotate("text", x = 0.735, y = 0.475, label = "2. ccsr_map()",
           size = 4.5, hjust = 0.5, fontface = "bold") +
  annotate("rect", xmin = 0.55, xmax = 0.92, ymin = 0.2, ymax = 0.35,
           fill = "white", color = "gray", linewidth = 0.5) +
  annotate("text", x = 0.735, y = 0.275, label = "3. Analysis Ready!",
           size = 4.5, hjust = 0.5, fontface = "bold") +
  annotate("segment", x = 0.735, xend = 0.735, y = 0.6, yend = 0.55,
           arrow = arrow(length = unit(0.2, "cm")), color = "#388E3C", linewidth = 1.5) +
  annotate("segment", x = 0.735, xend = 0.735, y = 0.4, yend = 0.35,
           arrow = arrow(length = unit(0.2, "cm")), color = "#388E3C", linewidth = 1.5) +
...
cat("Creating Figure 3: Output Format Comparison...\n")

# Panel A: Long format (ICD-10-CM and CCSR side-by-side; example ICD-10-CM E11.0 per HCUP CCSR)
panel_a <- ggplot() +
  coord_cartesian(xlim = c(0, 1), ylim = c(0, 1)) +
  ...
  annotate("text", x = 0.5, y = 0.15, label = "One row per ICD-10-CM row\nper assigned CCSR category",
           size = 2.9, hjust = 0.5) +
  theme_void() +
  theme(plot.background = element_rect(fill = "white"))

# Panel B: Wide format (slots follow HCUP CCSR Category 1–6; example E11.0)
panel_b <- ggplot() +
  coord_cartesian(xlim = c(0, 1), ylim = c(0, 1)) +
  annotate("rect", xmin = 0, xmax = 1, ymin = 0, ymax = 1,
           fill = "#E8F5E9", color = "black", linewidth = 1) +
...
# Panel C: Default only (inpatient default category for principal-style summaries)
panel_c <- ggplot() +
  coord_cartesian(xlim = c(0, 1), ylim = c(0, 1)) +
  annotate("rect", xmin = 0, xmax = 1, ymin = 0, ymax = 1,
           fill = "#FFF3E0", color = "black", linewidth = 1) +
...
cat("Creating Figure 4: HCUPtools Workflow...\n")
...
p4 <- ggplot() +
  ...
  # Step 2: Download Trend Tables (optional)
...
cat("Creating Figure 5: Performance Comparison...\n")
...
cat("- package_architecture.png\n")
cat("- workflow_comparison.png\n")
cat("- output_formats.png\n")
cat("- hcuptools_workflow.png\n")
cat("- performance_comparison.png\n")
